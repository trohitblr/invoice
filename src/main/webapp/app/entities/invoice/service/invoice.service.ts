...
export type PartialUpdateInvoice = Partial<IInvoice> & Pick<IInvoice, 'invoiceId'>;

type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'createdOn' | 'updatedOn'> & {
...
export class InvoiceService {
  ...
  getInvoiceIdentifier(invoice: Pick<IInvoice, 'invoiceId'>): number {
    return invoice.invoiceId;
  }

  compareInvoice(o1: Pick<IInvoice, 'invoiceId'> | null, o2: Pick<IInvoice, 'invoiceId'> | null): boolean {
    return o1 && o2 ? this.getInvoiceIdentifier(o1) === this.getInvoiceIdentifier(o2) : o1 === o2;
  }

  addInvoiceToCollectionIfMissing<Type extends Pick<IInvoice, 'invoiceId'>>(
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...invoice,
      createdOn: invoice.createdOn?.toJSON() ?? null,
      updatedOn: invoice.updatedOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...restInvoice,
      createdOn: restInvoice.createdOn ? dayjs(restInvoice.createdOn) : undefined,
      updatedOn: restInvoice.updatedOn ? dayjs(restInvoice.updatedOn) : undefined,
    };
  }
